...
exports.postLogin = (req, res, next) => { //
  ...
  passport.authenticate('local', (err, user, info) => { //uses localauth to verify the emails and passwords
    ...
    req.logIn(user, (err) => {
      ...
      res.redirect(req.session.returnTo || '/feed/0') //If it is a success, renders the todos.ejs
    })
  })(req, res, next)
}
...
exports.postSignup = (req, res, next) => { //
  ...
  User.findOne({
    $or: [ //Checks the database for a matching user in the database using the user schema...
    ...
    user.save((err) => { //if the user exists
      ...
      req.logIn(user, (err) => { //Make a login request
        ...
        res.redirect('/feed/0') //Sends you to the todos page
      })
    })
  })
}
